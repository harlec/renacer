...
<body class="mobile dashboard">
	<div class="">
		...
		<div class="kbg">
			<form id="frmfactura" method="post" action="inc/notadcf_e.php" >
				<div class="cuerpo">
					<div class="titulo">
						<h3>Factura</h3>
					</div>
					...
					<div class="container-fluid">
						<div class="row">
							...
							<input type="hidden" name="numero_compro" value="<?php echo $num_compro; ?>">
					  		<div class="col-sm-12">
					  			<input class="form-control" type="text" name="ruc" id="ruc" placeholder="Ingrese Ruc" value="<?php echo $rucn; ?>"><br>
					  		</div>
						  	<div class="col-sm-12">
...
//borrar item
			...
			$("body").on('click', '#facturar', function () {
		    	...
		    	$.ajax({
					...
					type: "POST",
					dataType: "json",
					url: "inc/notadcf_e.php",
					data: str2,
					success: function(response){
						if (response && response.ok) {
							window.open(response.enlace_del_pdf, '_blank');
							$btn.text('✓ Nota de crédito generada');
							if (response.aviso) alert(response.aviso);
						} else {
							alert(response && response.mensaje ? response.mensaje : 'No se pudo generar la nota de crédito.');
							$btn.prop('disabled', false);
						}
					},
					error: function(){
						alert('Error de conexión al generar la nota de crédito. Intenta de nuevo.');
						$btn.prop('disabled', false);
					}
				});
