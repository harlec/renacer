...
if (!is_array($leer_respuesta) || isset($leer_respuesta['errors']) || empty($leer_respuesta['numero'])) {
    ...
    // Guarda el documento en la ficha del cliente de esta venta, para no tener que
    // volver a pedírselo la próxima vez que se le emita un comprobante (solo si de
    // verdad escribieron un documento, no el "-" por defecto de boleta a "VARIOS").
    $ruc_normalizado = trim($ruc);
    if ($ruc_normalizado !== '' && $ruc_normalizado !== '-') {
