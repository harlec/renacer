...
foreach ($ventas_list as $value) {
	...
	$datos .='<tr><td>'.$value['id_variante'].'</td> 
    			<td id ="'.$value['id_variante'].'" class="var">'.$value['variante'].'</td> 
    			<td id ="'.$value['cantidad'].'" class="can">'.$value['cantidad'].'</td> 
    			<td><button class="editar_c btn-custom" alt="editar" value="'.$value['id_variante'].'"><img src="assets/img/edit.png"/></td> 
    		  </tr>';
    $i++;
}
...
<body class="mobile dashboard">
				...
				<div class="titulo">
					<div class="container-fluid">
						<div class="row">
							...
							<div class="col-xs-3">
								<button id="addnew" style="margin-top:6px;" class="btn btn-success">+ Agregar variante</button>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="col-xs-12">
							<form>
								<div class="form-group">
								    ...
								    <input class="form-control" type="text" name='variante' id="variante"><br>
	...
	$(document ).ready(function() {
	    ...
	    $('body').on('click',"#guardar_cate", function() {
	    	...
	    	var id_ca = $('#id').val();
	    	//alert(categoria);
	    	var str1 = 'variante=' + variante + '&cantidad=' + cantidad+'&accion=' + opcion+'&id=' + id_ca;
	    	alert(str1);
		  	$.ajax({	
		    	type: "POST",
				dataType: 'json',
			  	url: '/inc/registrar_variante.php',
			  	data: str1,
			  	success: function(data1) {
			  		...
			  		$('#cantidad').val('');
			   	 	...
			   	 	if (data1.accion =='nuevo') {
				   	 	table1.rows.add(
					       [[ data1.id, data1.variante, data1.cantidad,"<button>x</button>" ]]
					    ).draw();
			   	 	}
			   	 	else{
	   	...
	   	$('body').on('click',".editar_c", function() {
	    	...
	    	//alert(id_categoria);
	    	var nom_categoria1 = $(this).closest('tr').find('.var').text();
	    	var nombre = $(this).closest('tr').find('.can').text();
	    	$('#tituc').text('Editar color');
	    	$('#variante').val(nom_categoria1);
	    	$('#cantidad').val(nombre);
